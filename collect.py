...
def save_snapshot_json(buses, lines, poll_time, poll_num, day_folder):
    ...
    # Structure: snapshots/001/001.json (day_folder/poll_num)
...
def save_snapshot_csv(buses, poll_time, poll_num, day_folder, folder, include_stale=True):
    ...
    # Structure: csv_snapshots/001/001.csv (day_folder/poll_num)
...
def main():
    ...
    if buses:
        ...
        print(f"  Snapshot: snapshots/{day_folder}/{snap_file.name}")
        ...
        print(f"  CSV (all): csv_snapshots/{day_folder}/{csv_file.name}")
        ...
        print(f"  CSV (active): csv_active/{day_folder}/{csv_active.name} ({active_count} buses)")
